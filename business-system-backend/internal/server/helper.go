...
//nolint:errcheck
func NewServer(app *config.AppConfig, log *logrus.Entry, fcs ...func(*gin.Engine) error) *Serve {
	srv := &Serve{
		app:    app,
		logger: log,
	}
	...
	err := srv.RgistryRoute(fcs...)
	if err != nil {
		log.WithError(err).Fatal("register init route failed.")
	}
