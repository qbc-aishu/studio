...
export default class LoginClusterBase extends WebComponent<Props, State> {
        ...
        setTimeout(() => {
            if (this.refs.select) {
                (this.refs.select as any).src = `${
                    prefix ? prefix : ""
                }/interface/deployweb/login?lang=${
                    lang ? lang : Languages[0].language
                }&state=${random}&x-forwarded-prefix=${prefix}&product=deploy&_t=${new Date().getTime()}`;
            }
            ...
            if (
                ...
                if (this.refs.select) {
                    (this.refs.select as any).src = `${
                        ...
                        this.prefix
                    }&product=deploy&_t=${new Date().getTime()}`;
                }
                localStorage.setItem("oauth.signin.client.id", random);
            }
        });
    ...
    protected catchIFrameError({ target }: any) {
        const loginError = session.take("deploy.loginerror");
        if (loginError) {
            this.setState({
            ...
            const { innerText, outerHTML, outerText, textContent } =
                target.contentDocument.body;
            if (
                [innerText, outerHTML, outerText, textContent].some((text) => {
