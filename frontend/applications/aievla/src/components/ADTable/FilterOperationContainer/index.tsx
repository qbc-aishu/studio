  ...
  const showOptions = useCallback(() => {
    ...
    if (moreOptions.length <= perPageTotal) {
      const res = moreOptions?.map(item => {
        ...
        return (
          <div key={String(label) + cur} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            {label ? (
            ...
            <Form.Item className='ad-mr-3' name={id}>
              {itemDom === null ? (
                <Select
