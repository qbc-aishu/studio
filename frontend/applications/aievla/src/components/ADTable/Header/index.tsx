...
const Header: React.FC<HeaderProps> = props => {
  ...
  const handleList = (list: ButtonConfigProps[]) => {
    return list.map((buttonItem, index) => {
      const { key, type, label, orderMenu = [], orderField, order, onOrderMenuClick = () => {}, onHandle = () => {}, itemDom = null, disabled } = buttonItem;
      const menuRule = (
        <Menu selectedKeys={[orderField!]} className='ad-table-orderMenu' onClick={e => onOrderMenuClick?.(e)}>
          {_.map(orderMenu, (item: any) => {
            ...
            return (
              <Menu.Item key={id}>
                <div className='ad-table-orderMenu-icon'>{isSelected && <IconFont type='icon-fanhuishangji' className={iconDirection} />}</div>
                <div>{intlText}</div>
              </Menu.Item>
            );
          })}
      ...
      switch (type) {
        case 'add':
          return (
            <Format.Button key='add' type='primary' className='add-btn ad-mr-3' onClick={e => onHandle(e)}>
            ...
            <ContainerIsVisible isVisible={true} key={key}>
              <Tooltip key='add-down' title={label} placement='bottom' visible={false}>
                <Dropdown overlay={menuRule} trigger={['click']} placement='bottomRight'>
                  <Button type='primary' className='add-down-btn ad-mr-3' onClick={e => onHandle(e)}>
                    <IconFont type='icon-Add' style={{ color: '#fff' }} />
                    {label}
                    <CaretDownOutlined />
                  </Button>
          ...
          return (
            <ContainerIsVisible isVisible={true} key={key}>
              <Tooltip key='delete' title={label} placement='bottom' visible={false}>
                <Button disabled={disabled} className='delete-btn ad-mr-3' onClick={e => onHandle(e)}>
        ...
        case 'order':
          return (
            <Dropdown key='order' overlay={menuRule} trigger={['click']} placement='bottomRight' getPopupContainer={triggerNode => triggerNode.parentElement!}>
        ...
        case 'fresh':
          return (
            <Format.Button key='fresh' type='icon' tip={intl.get('global.refresh')} tipPosition='top' onClick={e => onHandle(e)}>
      ...
      <div className={classnames('ad-table-header', className)}>
        {children || (
          <>
            ...
            {/* 左右按钮容器 */}
            {
              <div className='ad-mb-4' style={{ display: 'flex', justifyContent: 'space-between', alignContent: 'center' }}>
                  ...
                  <ContainerIsVisible isVisible={!isFilter_}>
                    {showSearch && !isFilter_ ? (
                      <SearchInput
                        ...
                        className='ad-mr-3 search-input'
                    ...
                    {filterToolsOptions.slice(0, 2).map(item => {
                      ...
                      return (
                        <div key={id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} className='ad-mr-3'>
                          {label ? (
                            <span className='ad-ellipsis' style={{ flexShrink: 0, marginRight: 10 }} title={label}>
                              {label}
                            </span>
                          ) : null}
