...
const ITable: React.FC<ITableProps> = props => {
  ...
  const handleColumn = (cols: ColumnType<any>[]) => {
    ...
    const hasWidth = withWidthCols.reduce((pre, cur) => pre + Number(cur.width), 0);
  ...
  const {
    ...
    minConstraints: 60,
    defaultWidth: resetProps.rowSelection ? lastColWidth + Number(resetProps.rowSelection.columnWidth ?? 32) : lastColWidth,
    columnsState: !colsID ? undefined : { persistenceKey: `ADTableCols-${colsID}`, persistenceType: 'sessionStorage' },
  });
  ...
  useDeepCompareEffect(() => {
    if (isAutoCalculateScrollY && paginationConfig) {
      if (paginationConfig.total! > paginationConfig.pageSize!) onResize();
    }
  ...
  const getLocale = () => {
    ...
    if (searchValue || filterValue) showEmptyData = true;
    if (showEmptyData) {
      return {
        emptyText: (
  ...
  const targetColumns = useMemo(() => {
    if (resizableColumns && resizableColumns.length > 0) {
      resizableColumns[resizableColumns.length - 1].render = resizableColumns[resizableColumns.length - 1].render || renderCell;
  ...
  const renderContent = () => {
    return (
      <div
        ...
        className='ad-table-container ad-flex-item-full-height'
        id={domId.current}
      >
        <Dropdown
          placement='bottomLeft'
          overlay={() => {
            if (menuPos === 'BODY_MENU') return bodyContextMenu;
          ...
          <div ref={tableWrapperRef} onScrollCapture={onScrollCapture}>
            <Table
              ...
              rowClassName={(record, index, indent) => {
                ...
                const rowCls = typeof rowClassName === 'function' ? rowClassName?.(record, index, indent) : rowClassName;
  ...
  const getDomHeight = (domElement: Element) => {
    ...
    const elementHeight = parseFloat(computedStyle.height) + parseFloat(computedStyle.marginTop) + parseFloat(computedStyle.marginBottom);
    return elementHeight || 0;
  };
  const onResize = _.debounce(() => {
    let scrollY = 0;
    const domRefDom = document.getElementById(domId.current);
    const tableTitleDom = tableWrapperRef.current?.querySelector('.ant-table-title');
    const tableHeaderDom = tableWrapperRef.current?.querySelector('.ant-table-thead');
    const tablePaginationDom = tableWrapperRef.current?.querySelector('.ant-pagination'); // 52px
    if (domRefDom) {
  ...
  return isAutoCalculateScrollY ? <AdResizeObserver onResize={onResize}>{renderContent()}</AdResizeObserver> : renderContent();
};
