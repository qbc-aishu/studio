...
const Button = (props: ButtonType) => {
  ...
  const renderButton = () => {
    ...
    if (type === 'icon') {
      return (
        <Tooltip title={tip} placement={tipPosition} trigger={['hover']} arrowPointAtCenter>
          <span
            {...othersProps}
    ...
    if (type === 'icon-text') {
      ...
      const flag_3 = childrenList.length === 3 && typeof childrenList[0] === 'object' && typeof childrenList[2] === 'object';

      return (
        <Tooltip title={tip} placement={tipPosition} trigger={['hover']}>
          <span
            {...othersProps}
            className={classnames({ 'ad-btn-icon-text': flag_1 }, { 'ad-btn-text-icon': flag_2 }, { 'ad-btn-icon-text-icon': flag_3 }, className, {
    ...
    return (
      <Tooltip title={tip} placement={tipPosition} trigger={['hover']}>
        <AntdButton className={classnames(extendSize, className, 'ad-format-button')} {...othersProps} type={type as any}>
          {children}
        </AntdButton>
      </Tooltip>
    );
