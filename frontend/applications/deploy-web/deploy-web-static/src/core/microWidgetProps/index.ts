    ...
    return {
        ...
        token: {
            ...
            refreshOauth2Token: async () => {
                ...
                return {
                    access_token: cookie.get("deploy.oauth2_token") || "",
                    id_token: cookie.get("deploy.id_token") || "",
                    refresh_token: cookie.get("deploy.refresh_token") || "",
                };
            },
            get getToken() {
                return {
        ...
        history: {
            async getBasePathByName(microWidgetName: string) {
                ...
                return `${prefix}/deploy${micoroWidgetInfo?.app?.pathname}`;
            },
            navigateToMicroWidget({
                ...
                window.open(
                    `${location.protocol}//${location.hostname}${prefix}/deploy${micoroWidgetInfo?.app?.pathname}${path}`,
